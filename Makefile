...
looks: zsh kitty tmux themes background neofetch
...
zsh: check-git check-zsh-dependencies
...
kitty: check-kitty-dependencies check-fonts-powerline
	sudo apt-get install -y kitty fonts-powerline
	cp -r kitty ~/.config/

tmux: check-git check-tmux-dependencies
